...
if( ! class_exists( 'avia_sc_portfolio' ) )
{
	class avia_sc_portfolio extends aviaShortcodeTemplate
	{
		...
		function extra_assets()
		{
			...
			//load css
			wp_enqueue_style( 'avia-module-portfolio', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/portfolio/portfolio{$min_css}.css", array( 'avia-layout' ), $ver );

			//load js
			wp_enqueue_script( 'avia-module-isotope', AviaBuilder::$path['pluginUrlRoot'] . 'avia-shortcodes/portfolio/isotope.min.js', array( 'avia-shortcodes' ), $ver , true );
			wp_enqueue_script( 'avia-module-portfolio', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/portfolio/portfolio{$min_js}.js", array( 'avia-shortcodes' ), $ver , true );
...
//
			...
			$c = array(
						...
						array(
								...
								'lockable'		=> true,
								'subtype'		=> array(
														'default'	=> AviaHtmlHelper::number_array( 2, 6, 1, array( __( '1 column', 'avia_framework' ) => '1' ), ' ' . __( 'columns', 'avia_framework' ) ),
														'medium'	=> AviaHtmlHelper::number_array( 2, 4, 1, array( __( 'Use theme default (3 columns)', 'avia_framework' )	=> '', __( '1 column', 'avia_framework' ) => '1' ), ' ' . __( 'columns', 'avia_framework' ) ),
														'small'		=> AviaHtmlHelper::number_array( 2, 4, 1, array( __( 'Use theme default (2 columns)', 'avia_framework' )	=> '', __( '1 column', 'avia_framework' ) => '1' ), ' ' . __( 'columns', 'avia_framework' ) ),
														'mini'		=> AviaHtmlHelper::number_array( 2, 4, 1, array( __( 'Use theme default (1 column)', 'avia_framework' )	=> '', __( '1 column', 'avia_framework' ) => '1' ), ' ' . __( 'columns', 'avia_framework' ) )
													),
								'std'			=> array(
														'default'	=> '4',
														'medium'	=> '',
						...
						array(
								...
								'desc' 	=> __( 'Choose the 1 column layout. This will ignore all responsive settings you make.', 'avia_framework' ),
		...
		protected function sort_buttons( $entries, $params )
		{
			...
			foreach( $categories as $category )
			{
				if( in_array( $category->term_id, $current_page_cats ) )
				{
					...
					$output .=		'<span class="inner_sort_button">';
					$output .= 			'<span>' . esc_html( trim( $category->cat_name ) ) . '</span>';
					$output .= 			'<small class="av-cat-count"> ' . $cat_count[ $category->term_id ] . ' </small>';
					$output .=		'</span>';
					$output .= 	'</a>';
				}
			}

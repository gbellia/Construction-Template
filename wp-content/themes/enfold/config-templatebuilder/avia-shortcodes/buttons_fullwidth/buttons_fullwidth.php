...
if ( ! class_exists( 'avia_sc_button_full' ) )
{
	class avia_sc_button_full extends aviaShortcodeTemplate
	{
		...
		function extra_assets()
		{
			...
			wp_enqueue_style( 'avia-module-button-fullwidth', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/buttons_fullwidth/buttons_fullwidth{$min_css}.css", array( 'avia-layout' ), $ver );
		}
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			$html .=	"<a href='{$link}' class='{$container_class}' {$blank} {$title_attr}>";
			$html .=		$content_html;
			$html .=		$background_hover;
			$html .=	'</a>';

			$output =  "<div {$meta['custom_el_id']} class='avia-button-wrap avia-button-{$atts['position']} {$meta['el_class']}'>{$html}</div>";

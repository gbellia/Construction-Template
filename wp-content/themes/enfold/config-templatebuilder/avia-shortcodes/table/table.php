...
if( ! class_exists( 'avia_sc_table' ) )
{
	class avia_sc_table extends aviaShortcodeTemplate
	{
		...
		function admin_assets()
		{
			...
			wp_register_script( 'avia_table_js', AviaBuilder::$path['assetsURL'] . "js/avia-table{$min_js}.js", array( 'avia_modal_js' ), $ver, true );
		...
		function extra_assets()
		{
			...
			//load css
			wp_enqueue_style( 'avia-module-table', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/table/table{$min_css}.css", array( 'avia-layout' ), $ver );
		}
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			if( empty( $result['atts']['table_rows'] ) )
			{
				return '';
			}
		...
		protected function pricing_table( array &$atts, array $meta, $element_id )
		{
			...
			$output  = '';
			$output .= $style_tag;
			$output .= "<div {$meta['custom_el_id']} class='{$container_class}' {$markup}>";

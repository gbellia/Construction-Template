...
if( ! class_exists( 'avia_sc_image_hotspots' ) )
{
	class avia_sc_image_hotspots extends aviaShortcodeTemplate
	{
		...
		function extra_assets()
		{
			...
			//load css
			wp_enqueue_style( 'avia-module-hotspot', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/image_hotspots/image_hotspots{$min_css}.css", array( 'avia-layout' ), $ver );

				//load js
			wp_enqueue_script( 'avia-module-hotspot', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/image_hotspots/image_hotspots{$min_js}.js", array( 'avia-shortcodes' ), $ver, true );
		}
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			if( ! empty( $src ) )
			{
				...
				$output .= $style_tag;
				$output .= $item_tag;
				$output .= "<div {$el_id} class='{$container_class} {$extra_class}' {$markup_img}>";
			...
			$skipSecond = false;
			$params['class'] = "main_color av-fullwidth-hotspots {$meta['el_class']} " . $element_styling->responsive_classes_string( 'hide_element', $atts );

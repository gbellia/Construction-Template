...
if( ! class_exists( 'avia_sc_icon_box' ) )
{
	class avia_sc_icon_box extends aviaShortcodeTemplate
	{
		...
		function extra_assets()
		{
			...
			wp_enqueue_style( 'avia-module-iconbox', AviaBuilder::$path['pluginUrlRoot'] . "avia-shortcodes/iconbox/iconbox{$min_css}.css", array( 'avia-layout' ), $ver );
		}
...
//			}
		...
		function shortcode_handler( $atts, $content = '', $shortcodename = '', $meta = '' )
		{
			...
			$output  = '';
			$output .= $style_tag;
			$output .= "<article {$meta['custom_el_id']} class='{$container_class}' {$markup_entry}>";
			...
			if( $title )
			{
				$output .= 			"<{$heading} class='iconbox_content_title {$css}' {$markup_title}>{$title}</{$heading}>";
			}
			$output .= 			'</header>';


			$output .= 			"<div class='iconbox_content_container {$content_class}' {$markup_content}>";

...
//			$this->output .= $rule['elements'] . ".{$font_css} {font-family:'" . $font['family'] ."', 'HelveticaNeue', 'Helvetica Neue', Helvetica, Arial, sans-serif;" . $font_weight . "}";
			...
			/**
			 * Add theme options font family as CSS variables
			 *
			 * @since 5.2
			 * @param boolean $supress_variables
			 * @return boolean
			 */
			if( false === apply_filters( 'avf_supress_css_theme_variables', false ) && isset( $rule['font_source'] ) )
			{
				switch( $rule['font_source'] )
				{
					case 'google_webfont':
						$var_key = 'heading';
						break;
					case 'default_font':
						...
						break;
					default:
						$var_key = 'source-' . $rule['font_source'];
				}

				$this->output .= "

:root {
--enfold-font-family-{$var_key}: {$family};
}
